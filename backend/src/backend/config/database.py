...
def create_extensions():
    """Create default extensions if they don't exist."""
    from backend.models.system import Extension
    
    with Session(engine) as session:
        extensions_data = [
            {"name": "Views", "slug": "views", "version": "", "is_active": True, "config": {}},
            {"name": "Comments", "slug": "comments", "version": "", "is_active": True, "config": {}},
            {"name": "Tags", "slug": "tags", "version": "", "is_active": True, "config": {}},
            {"name": "Likes", "slug": "likes", "version": "", "is_active": True, "config": {}},
        ]
        
        for ext_data in extensions_data:
            # Check if extension already exists
            existing = session.exec(
                select(Extension).where(Extension.slug == ext_data["slug"])
            ).first()
            
            if not existing:
                extension = Extension(**ext_data)
                session.add(extension)
                print(f"Created extension: {ext_data['name']}")
